...
const { Farmer, validate } = require("../models/users");
...
router.post('/',async(req,res)=>{
    try {
        ...
        if(req.body.password=== req.body.Confirm_password){
            ...
            user= await new Farmer({...req.body, password:hashpassword, Confirm_password:hashCpassword}).save();
            // const token = await new Token({
            //     userId: user._id,
            //     token: crypto.randomBytes(32).toString("hex"),
            // }).save();
            // const url = `${process.env.BASE_URL}FarmerRoutes/${user.id}/verify/${token.token}`;
...
// 		await Farmer.updateOne({_id: user._id},{
//             $set:{
//                 isverified:true  
//             }
//         });
